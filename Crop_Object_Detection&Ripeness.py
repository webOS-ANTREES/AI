...
def process_images(model, folder_path, save_path, classes_to_filter=(0, 1, 3)):
    ...
    # 전체 개수 및 각 클래스별 개수를 추적할 변수
    total_strawberry_count = 0
    total_pest_count = 0
    total_ripe_count = 0  # 익은 딸기 개수
    total_unripe_count = 0  # 안 익은 딸기 개수
    ...
    for img_path in images:
        ...
        total_strawberry_count += len(detections)
                ...
                for idx, line in enumerate(lines):
                    ...
                    if x_max - x_min > 0 and y_max - y_min > 0:
                        ...
                        if label_class == 0:
                            ...
                            mqtt_client.publish(TOPIC,"{}, {}, {}, {}, {}".format(position, forward_or_reverse,x_center,y_center,y_max-y_min))
                            # 클래스 0을 7로 변경
                            label_class = 7
                        elif label_class == 1:
...
if __name__ == "__main__":
    model_path = 'C:/Users/antl/yolov5/runs/train/exp12/weights/best.pt'
    folder_path = 'C:/Users/antl/SW_22th_contest/Object_Detective_Data/collect_images'
    berry_folder_path = 'C:/Users/antl/SW_22th_contest/Object_Detective_Data/processed_images/captured_ripe_berry'
    save_path = 'C:/Users/antl/SW_22th_contest/Object_Detective_Data/processed_images'

    mqtt_client = setup_mqtt()
    model = load_model(model_path)

    while True:    
        #save_path에 원본파일, 라벨파일, 객체탐색결과파일이렇게 3개 생김.
        #if 통신 받은게 있으면 - process_images,capture_bounding_boxes, detect_pests_in_images 실행
        # 폴더에 파일이 있는지 확인 (폴더가 비어있지 않으면 실행)
        if os.listdir(folder_path):  # 폴더가 비어있지 않다면 리스트가 비어있지 않음
